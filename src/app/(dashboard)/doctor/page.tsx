  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-3">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <Button
            ...
            className="h-10 w-10 rounded-xl text-muted-foreground hover:text-gray-600"
            title="Refresh"
          >
      ...
      {nextPatient && patientHistory.length > 0 && (
        <div className="bg-card rounded-2xl border border-border/60 p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="text-[13px] font-bold text-foreground">Recent Visits &mdash; {nextPatient.patient_name}</h3>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {patientHistory.map((visit) => (
              <div key={visit.prescription_id} className="rounded-xl bg-muted/50 p-3 space-y-1.5">
                <p className="text-[11px] font-bold text-cyan-700">{formatDate(visit.date)}</p>
                {visit.symptoms && (
                  <p className="text-[11px] text-muted-foreground"><span className="font-semibold text-foreground">Symptoms:</span> {visit.symptoms.length > 60 ? visit.symptoms.slice(0, 60) + "..." : visit.symptoms}</p>
                )}
                {visit.diagnosis && (
                  <p className="text-[11px] text-muted-foreground"><span className="font-semibold">Dx:</span> {visit.diagnosis.length > 60 ? visit.diagnosis.slice(0, 60) + "..." : visit.diagnosis}</p>
                )}
                {visit.medicines && visit.medicines.length > 0 && (
                  <p className="text-[11px] text-muted-foreground">
                    <span className="font-semibold text-foreground">Rx:</span> {visit.medicines.slice(0, 3).map((m) => m.name).join(", ")}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-5 min-h-[420px]">
        ...
        <div className="flex flex-col bg-[#F7F8FA] dark:bg-card rounded-2xl overflow-hidden border border-border/60/60">
          <div className="flex items-center justify-between px-4 py-3 shrink-0">
            <div className="flex items-center gap-2">
              <div className="w-2.5 h-2.5 rounded-full bg-indigo-500" />
              <span className="text-[13px] font-bold text-foreground">In Consultation</span>
            </div>
            <span className="text-[11px] font-bold px-2 py-0.5 rounded-full tabular-nums bg-indigo-50 dark:bg-indigo-950/20 text-muted-foreground">
            ...
            <div className="px-2.5 pt-2.5 pb-2.5 space-y-2.5">
              {inConsultation.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  ...
                  <p className="text-xs font-medium text-muted-foreground">No active consultations</p>
                </div>
                ...
                <motion.div
                  ...
                  className="group relative bg-card rounded-xl shadow-sm hover:shadow-md transition-all border border-transparent hover:border-indigo-200 dark:hover:border-indigo-800/40"
                >
                  ...
                  <div className="p-3.5">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-bold text-foreground truncate">{entry.patient_name}</p>
                        <p className="text-[11px] text-muted-foreground truncate mt-0.5">{formatPhone(entry.patient_phone)}</p>
                      </div>
                    </div>
        ...
        <div className="flex flex-col bg-[#F7F8FA] dark:bg-card rounded-2xl overflow-hidden border border-border/60/60">
          <div className="flex items-center justify-between px-4 py-3 shrink-0">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[13px] font-bold text-foreground">Waiting</span>
            </div>
            <span className="text-[11px] font-bold px-2 py-0.5 rounded-full tabular-nums bg-sky-50 dark:bg-sky-950/20 text-muted-foreground">
              {waiting.length}
            </span>
          </div>
          <ScrollArea className="flex-1 max-h-[520px]">
            <div className="px-2.5 pt-2.5 pb-2.5 space-y-2.5">
              {waiting.length === 0 ? (
                ...
                <motion.div
                  ...
                  transition={{ delay: i * 0.04 }}
                  className={cn(
                    "group relative bg-card rounded-xl shadow-sm hover:shadow-md transition-all cursor-pointer",
                  ...
                  <div className="p-3.5">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[11px] text-muted-foreground truncate mt-0.5">{formatPhone(entry.patient_phone)}{entry.walk_in ? " \u00b7 Walk-in" : ""}</p>
                      </div>
                    </div>
                    <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-50 dark:border-gray-800">
                      <div className="flex items-center gap-1.5">
        ...
        <div className="flex flex-col bg-[#F7F8FA] dark:bg-card rounded-2xl overflow-hidden border border-border/60/60">
          <div className="flex items-center justify-between px-4 py-3 shrink-0">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[13px] font-bold text-foreground">Upcoming</span>
            </div>
            ...
            <div className="px-2.5 pt-2.5 pb-2.5 space-y-2.5">
              {upcoming.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  ...
                  <p className="text-xs font-medium text-muted-foreground">No upcoming appointments</p>
                </div>
                ...
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ delay: i * 0.04 }}
                  className="bg-card rounded-xl shadow-sm hover:shadow-md transition-all border border-transparent hover:border-cyan-200 dark:hover:border-cyan-800/40"
                >
                  <div className="p-3.5 flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-bold text-foreground truncate">{apt.patient_name}</p>
      ...
      {/* ══════ COMPLETED ══════ */}
      {completed.length > 0 && (
        <div className="bg-[#F7F8FA] dark:bg-card rounded-2xl border border-border/60/60 p-4">
          <div className="flex items-center gap-2 mb-3">
            <div className="w-2.5 h-2.5 rounded-full bg-green-400" />
            <span className="text-[13px] font-bold text-foreground">Completed Today</span>
            <span className="text-[11px] font-bold px-2 py-0.5 rounded-full tabular-nums bg-green-50 dark:bg-green-950/20 text-muted-foreground ml-auto">
              {completed.length}
            </span>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
            {completed.slice(0, 6).map((entry) => (
              <div key={entry.queue_id} className="bg-card rounded-xl p-3 flex items-center gap-3 border border-transparent opacity-70 hover:opacity-100 transition-all">
                <div className="w-9 h-9 rounded-xl bg-green-50 dark:bg-green-950/30 flex items-center justify-center">
                  <CheckCircle2 className="w-4 h-4 text-green-500" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-muted-foreground dark:text-muted-foreground truncate">{entry.patient_name}</p>
                  <p className="text-[11px] text-muted-foreground truncate">{formatPhone(entry.patient_phone)}</p>
                </div>
              </div>
            ))}
          </div>
