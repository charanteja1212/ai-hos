...
export function Topbar({ user, onToggleMobile }: TopbarProps) {
  ...
  const roleBadgeColor: Record<string, string> = {
    ...
    BRANCH_ADMIN: "bg-cyan-50 text-cyan-800 border-cyan-200 dark:bg-cyan-900/20 dark:text-cyan-300 dark:border-cyan-800",
    ADMIN: "bg-cyan-50 text-cyan-800 border-cyan-200 dark:bg-cyan-900/20 dark:text-cyan-300 dark:border-cyan-800",
          ...
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" className="gap-2 px-2">
              <Avatar className="w-8 h-8">
                <AvatarFallback className="text-xs bg-cyan-700 text-white font-semibold">
                  {initials}
                </AvatarFallback>
              </Avatar>
